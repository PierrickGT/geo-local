...
export function serializeValue(v: Value): StoredValue {
	switch (v.type) {
		...
		case 'integer':
			return { type: 'integer', payload: { value: v.value.toString(), unit: v.unit ? bufToBase64(v.unit) : undefined } }
		case 'float':
			return { type: 'float', payload: { value: v.value, unit: v.unit ? bufToBase64(v.unit) : undefined } }
		case 'decimal':
			return {
				type: 'decimal',
				payload: {
					exponent: v.exponent,
					mantissa: v.mantissa.type === 'i64'
		...
		case 'text':
			return { type: 'text', payload: { value: v.value, language: v.language ? bufToBase64(v.language) : undefined } }
		case 'bytes':
		...
		case 'rect':
			return { type: 'rect', payload: { minLat: v.minLat, minLon: v.minLon, maxLat: v.maxLat, maxLon: v.maxLon } }
		case 'embedding':
			return { type: 'embedding', payload: { subType: v.subType, dims: v.dims, data: bufToBase64(v.data) } }
		default: {
